    ...
    fun inputTextSearchToMaxMin(searchTextField: String): Pair<Int, Int> {
        try {
            when {
                ...
                searchTextField.contains(":") && searchTextField.contains(".") && searchTextField.length > 8 -> {
                    return Pair(0, 0)
                }
                searchTextField.endsWith(":") && searchTextField.length == 3 -> {
                    val minutes = searchTextField.dropLast(1).toInt()
                    return Pair(minutes * 60 * 1000, (minutes + 1) * 60 * 1000 - 1)
                }
                searchTextField.endsWith(":") -> {
                    val minutes = searchTextField.dropLast(1).toInt()
                    return Pair(minutes * 60 * 1000, (minutes + 1) * 60 * 1000 - 1)
                }
                searchTextField.length == 4 && searchTextField[2] == ':' -> {
                    val parts = searchTextField.split(":")
                    val min = parts[0].toInt() * 60 * 1000 + parts[1].toInt() * 10000
                    return Pair(min, min + 9999)
                }
                searchTextField.length == 3 && searchTextField[1] == ':' -> {
                    val parts = searchTextField.split(":")
                    val min = parts[0].toInt() * 60 * 1000 + parts[1].toInt() * 10000
                    return Pair(min, min + 9999)
                }
                searchTextField.length == 5 && searchTextField[2] == ':' -> {
                    val parts = searchTextField.split(":")
                    val min = parts[0].toInt() * 60 * 1000 + parts[1].toInt() * 1000
                    return Pair(min, min + 999)
                }
                searchTextField.contains(":") && searchTextField.length == 4 -> {
                    val parts = searchTextField.split(":")
                    val min = parts[0].toInt() * 60 * 1000 + parts[1].toInt() * 1000
                    return Pair(min, min + 999)
                }
                searchTextField.length == 6 && searchTextField.endsWith(".") && searchTextField.contains(":") -> {
                    val parts = searchTextField.dropLast(1).split(":")
                    val min = parts[0].toInt() * 60 * 1000 + parts[1].toInt() * 1000
                    return Pair(min, min + 999)
                }
                searchTextField.length == 5 && searchTextField.endsWith(".") && searchTextField.contains(":") -> {
                    val parts = searchTextField.dropLast(1).split(":")
                    val min = parts[0].toInt() * 60 * 1000 + parts[1].toInt() * 1000
                    return Pair(min, min + 999)
                }
                searchTextField.length == 7 && searchTextField[2] == ':' && searchTextField[5] == '.' -> {
                    val parts = searchTextField.split(":")
                    val timeParts = parts[1].split(".")
                    val min = parts[0].toInt() * 60 * 1000 + timeParts[0].toInt() * 1000 + timeParts[1].toInt() * 100
                    return Pair(min, min + 99)
                }
                searchTextField.contains(":") && searchTextField.contains(".") && searchTextField.length == 6 -> {
                    val parts = searchTextField.split(":")
                    val timeParts = parts[1].split(".")
                    val min = parts[0].toInt() * 60 * 1000 + timeParts[0].toInt() * 1000 + timeParts[1].toInt() * 100
                    return Pair(min, min + 99)
                ...
                searchTextField.contains(":") && searchTextField.contains(".") && searchTextField.length == 8 -> {
                    val parts = searchTextField.split(":")
                    val timeParts = parts[1].split(".")
                    val min = parts[0].toInt() * 60 * 1000 + timeParts[0].toInt() * 1000 + timeParts[1].toInt() * 10
                    return Pair(min, min + 9)
                }
                searchTextField.contains(":") && searchTextField.contains(".") && searchTextField.length == 7 -> {
                    val parts = searchTextField.split(":")
                    val timeParts = parts[1].split(".")
                    val min = parts[0].toInt() * 60 * 1000 + timeParts[0].toInt() * 1000 + timeParts[1].toInt() * 10
                    return Pair(min, min + 9)
                }
                searchTextField.length == 2 && searchTextField.endsWith(".") -> {
                    val seconds = searchTextField.dropLast(1).toInt()
                    return Pair(seconds * 1000, (seconds + 1) * 1000 - 1)
                }
                searchTextField.length == 3 && searchTextField.endsWith(".") -> {
                    val seconds = searchTextField.dropLast(1).toInt()
                    return Pair(seconds * 1000, (seconds + 1) * 1000 - 1)
                }
                //22.23
                searchTextField.matches(Regex("\\d+\\.\\d{2}")) -> {
                    val parts = searchTextField.split(".")
                    val seconds = parts[0].toInt()
                    val milliseconds = parts[1].toInt()
                    val min = seconds * 1000 + milliseconds * 10
                    return Pair(min, min + 9)
                }
                //22.2
                searchTextField.matches(Regex("\\d+\\.\\d{1}")) -> {
                    val parts = searchTextField.split(".")
                    val seconds = parts[0].toInt()
                    val milliseconds = parts[1].toInt()
                    val min = seconds * 1000 + milliseconds * 100
                    return Pair(min, min + 99)
                }
                //случай для чисел с точкой
                searchTextField.contains(".") -> {
                    val timeInMs = (searchTextField.toDouble() * 1000).toInt()
                    return Pair(timeInMs, timeInMs)
                }
                //секунды
                else -> {
                    val seconds = searchTextField.toInt()
